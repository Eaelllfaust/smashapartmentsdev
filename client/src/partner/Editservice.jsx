...
export default function UpdateAirportPickup({ listingId }) {
  ...
  }, [user, loading, navigate, listingId]);

  const fetchListingData = async (id) => {
    try {
      const response = await axios.get(`/airportpickuplisting/${id}`);
      ...
      setExistingImages(data.images.map((img) => ({ url: img.location, name: img.name })));
    } catch (error) {
  ...
  const handleImageRemove = (indexToRemove, type) => {
    if (type === "existing") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`/airportpickuplisting/${listingId}`, formData, {
        headers: {
        ...
        <section className="form_area_6">
          <div className="list_holder">
            <div className="list_1">
              <h2 className="support_text">Update images</h2>
              <br />
              ...
              <div className="image_preview">
                {existingImages.map((image, index) => (
                  <div key={`existing-${index}`} className="image_container">
                    <img src={`http://localhost:8000/${image.url}`} alt={`Existing ${index + 1}`} />
                    <button
            ...
            <div className="list_2 v85">
              <h2 className="support_text">Update information</h2>
              <br />
              <div className="form">
